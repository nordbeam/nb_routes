...
For automatic route regeneration during development, add the `nbRoutes` plugin to the Vite+ config generated by `nb_vite`:
...
Run the frontend through Vite+ (`vp dev`, `vp check`, and `vp build`). `lazyPlugins` keeps Phoenix and route-plugin startup out of format, lint, and type-check-only commands while loading them normally for development and builds.
